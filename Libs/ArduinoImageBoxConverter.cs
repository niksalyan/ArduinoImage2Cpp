...
public static class ArduinoImageBoxConverter
{
    ...
    public sealed class BoxImage
    {
        ...
        public string ToArduinoCode(DataModel data)
        {
            ...
            string identifier =
                MakeIdentifier(data.Output.Name).ToUpperInvariant();

            var sb = new StringBuilder();

            sb.AppendLine(@$"
// ============================================================
// Generated by ArduinoImageBoxConverter - {identifier}
// ============================================================

#define {identifier}_BOX_COUNT   {BoxCount}

const uint16_t {identifier}Palette[{ColorsCount}] PROGMEM{(data.ProgmemFar ? "_FAR" : "")} = {{
  {String.Join("", Palette.Select((p, i) => $"  0x{(ushort)ToRgb565(p):X4}{(i % 8 == 7 ? ",\r\n  " : ", ")}"))}
}};

const uint8_t {identifier}Boxes[] PROGMEM{(data.ProgmemFar ? "_FAR" : "")} = {{
  {String.Join("", Boxes.Select((b, i) => $"{b.X}, {b.Y}, {b.Width}, {b.Height}, {b.ColorIndex}{(i % 8 == 7 ? ",\r\n  " : ", ")}"))}
}};
...
static inline void {identifier}DrawImage(
    ...
    int16_t scaleY = 1)
{{
    uint_farptr_t boxesAddress = pgm_get_far_address({identifier}Boxes);
    uint_farptr_t paletteAddress = pgm_get_far_address({identifier}Palette);

    for (uint16_t i = 0; i < {identifier}_BOX_COUNT; i++)
    {{
        ...
        tft.{(fastApi ? "fastFillRect" : "fillRect")}(
            offsetX + x * scaleX,
            offsetY + y * scaleY,
            width * scaleX,
            height * scaleY,
